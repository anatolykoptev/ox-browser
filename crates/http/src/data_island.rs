...
//! as JSON inside `<script type="application/json">` tags rather than in
//! visible DOM elements. This module walks those JSON blobs and recovers
...
///
/// `dom_word_count` — word count of the DOM-extracted content; if above
/// `SPARSE_THRESHOLD`, the function returns `None` immediately.
/// `existing_markdown` — the markdown already extracted from the DOM, used
/// for deduplication.
pub fn try_extract(
    doc: &Document,
    dom_word_count: usize,
    ...
    for node in doc.select("script[type='application/json']").nodes() {
        if all_chunks.len() >= MAX_CHUNKS {
            break;
        }

        let json_text = node.text().to_string();
        if json_text.len() < 50 {
            continue;
        }
        ...
        if !chunks.is_empty() {
            let script_id = node.attr("id").map(|s| s.to_string()).unwrap_or_default();
            let data_target = node
                .attr("data-target")
                .map(|s| s.to_string())
                .unwrap_or_default();
            debug!(
    ...
    // Note: SvelteKit data islands are handled in structured_data.rs (#61)
...
mod tests {
    ...
    fn empty_html_returns_none() {
        ...
        assert!(try_extract(&doc, 0, "").is_none());
    }

    #[test]
    fn skips_non_json_script_tags() {
        let html = r#"<html><body>
    ...
    #[test]
    fn extracts_stat_arrays() {
        let html = r##"<html><body>
        <script type="application/json">
        {"stats":["100M+ active users worldwide", "#1 rated platform by Gartner", "Available in 50+ countries"]}
        </script>
        </body></html>"##;
        let doc = Document::from(html);
        let result = try_extract(&doc, 0, "").unwrap();
        assert!(result.contains("100M+ active users worldwide"));
        assert!(result.contains("#1 rated platform by Gartner"));
    }
}
