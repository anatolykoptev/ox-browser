...
//!
//! Ported from webclaw `crates/webclaw-core/src/llm/cleanup.rs` (MIT).
use once_cell::sync::Lazy;
use regex::Regex;

use super::noise_text;
...
fn is_ui_control_line(line: &str) -> bool {
    let trimmed = line.trim();
    if trimmed.is_empty() {
        return false;
    }
...
fn is_long_alt_description(line: &str) -> bool {
    let trimmed = line.trim();
...
fn is_css_artifact_line(trimmed: &str) -> bool {
    if trimmed.is_empty() {
        return false;
    }
...
pub(crate) fn strip_css_class_lines(input: &str) -> String {
    ...
    for line in input.lines() {
        ...
        if !is_structural && noise_text::is_css_class_text(trimmed) {
            continue;
        }
...
fn strip_trailing_css_classes(line: &str) -> String {
    ...
    for i in (0..words.len()).rev() {
        if noise_text::is_css_class_word_pub(words[i]) {
            last_content = i;
        } else {
            break;
        }
...
mod tests {
    ...
    fn long_alt_description_real_content_kept() {
        assert!(!is_long_alt_description("An illustration"));
        assert!(!is_long_alt_description(
