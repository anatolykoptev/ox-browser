...
//! JSON data island approach ([`crate::data_island`]) only handles
...
/// parsed [`Document`] should use [`extract_js_data_from_doc`] to avoid a second parse.
...
/// Execute inline `<script>` tags in a QuickJS sandbox and extract `window.__*` data,
/// reusing an already-parsed [`Document`] instead of re-parsing the HTML.
pub fn extract_js_data_from_doc(doc: &Document) -> Vec<JsDataBlob> {
...
mod tests {
    ...
    #[test]
    fn has_js_candidate_data_detects_markers() {
        assert!(has_js_candidate_data(
            r#"<script>window.__PRELOADED_STATE__ = {};</script>"#
        ));
        assert!(has_js_candidate_data(
            r#"<script>self.__next_f.push([1,"data"])</script>"#
        ));
        assert!(has_js_candidate_data(
            r#"<script type="application/json">{"data":1}</script>"#
        ));
        assert!(!has_js_candidate_data(
            r#"<html><body><p>just text</p></body></html>"#
        ));
    }
}
