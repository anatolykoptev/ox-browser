...
pub(super) fn check_xss_protection(h: &HashMap<String, String>, out: &mut Vec<HeaderFinding>) {
    ...
    match get(h, name) {
        None => out.push(present(
            name,
            "",
            Severity::Info,
            "X-XSS-Protection absent (correct, header is deprecated)",
        )),
        Some(v) if v.trim() == "0" => out.push(present(
            name,
            &v,
            Severity::Info,
            "X-XSS-Protection disabled (correct)",
        )),
        Some(v) => out.push(HeaderFinding {
            header: name.into(),
            status: HeaderStatus::Deprecated,
            value: Some(v),
            description: "X-XSS-Protection is deprecated and can introduce vulnerabilities".into(),
            severity: Severity::Low,
            recommendation: Some("Remove header or set to 0".into()),
        }),
    }
}
...
pub(super) fn check_reporting_endpoints(h: &HashMap<String, String>, out: &mut Vec<HeaderFinding>) {
    ...
    match get(h, name) {
        None => out.push(present(
            ...
            "",
            Severity::Info,
            "Reporting-Endpoints absent (optional)",
        )),
        Some(v) => out.push(present(
            name,
            &v,
            Severity::Info,
            "Reporting-Endpoints configured",
        )),
    }
}
...
pub(super) fn check_nel(h: &HashMap<String, String>, out: &mut Vec<HeaderFinding>) {
    ...
    match get(h, name) {
        ...
        Some(v) => out.push(present(
            ...
            &v,
            Severity::Info,
            "Network Error Logging configured",
        )),
    }
}

pub(super) fn check_cross_domain_policies(
        ...
        Some(v) => {
            ...
            if lower == "none" || lower == "master-only" {
                ...
                out.push(present(
                    ...
                    &v,
                    Severity::Low,
                    "Cross-domain policies may be too permissive",
                ));
            }
        }
    }
...
pub(super) fn check_dns_prefetch(h: &HashMap<String, String>, out: &mut Vec<HeaderFinding>) {
    ...
    match get(h, name) {
        None => out.push(present(
            ...
            "",
            Severity::Info,
            "X-DNS-Prefetch-Control absent (acceptable)",
        )),
        ...
        Some(v) => out.push(present(
            ...
            &v,
            Severity::Low,
            "DNS prefetch enabled — minor privacy concern",
        )),
    }
}
        ...
        None => {
            if has_cookies {
                out.push(HeaderFinding {
                    ...
                    value: None,
                    description: "Sensitive page (sets cookies) without Cache-Control: no-store"
                        .into(),
                    severity: Severity::Medium,
                    recommendation: Some(
                        "Add Cache-Control: no-store for pages that set cookies".into(),
                    ),
                });
            } else {
                out.push(present(
                    ...
                    "",
                    Severity::Info,
                    "Cache-Control absent (informational)",
                ));
            }
        }
        Some(v) => {
...
pub(super) fn check_clear_site_data(h: &HashMap<String, String>, out: &mut Vec<HeaderFinding>) {
    ...
    match get(h, name) {
        None => out.push(present(
            ...
            "",
            Severity::Info,
            "Clear-Site-Data absent (optional)",
        )),
        Some(v) => out.push(present(
            name,
            &v,
...
pub(super) fn check_content_type_charset(h: &HashMap<String, String>) -> Option<HeaderFinding> {
