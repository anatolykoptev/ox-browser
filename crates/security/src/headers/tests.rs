...
fn test_hsts_good() {
    ...
    assert!(f.description.contains("preload-ready"), "desc={}", f.description);
}
...
fn test_hsts_preload_ready() {
    let headers = h(&[("strict-transport-security", "max-age=63072000; includeSubDomains; preload")]);
    let report = analyze_headers(&headers, "https://example.com");
    let f = report.findings.iter().find(|f| f.header == "strict-transport-security").unwrap();
    assert_eq!(f.severity, Severity::Info);
    assert!(f.description.contains("preload-ready"), "desc={}", f.description);
...
fn test_hsts_preload_incomplete() {
    ...
    let f = report.findings.iter().find(|f| f.header == "strict-transport-security").unwrap();
    assert_eq!(f.severity, Severity::Low);
    assert!(f.description.contains("preload flag but doesn't meet"), "desc={}", f.description);
}
...
fn test_cache_control_missing_with_cookies() {
    ...
    let report = analyze_headers(&headers, "https://example.com");
    let f = report.findings.iter().find(|f| f.header == "cache-control").unwrap();
    assert_eq!(f.severity, Severity::Medium);
    assert!(f.description.contains("Sensitive page"), "desc={}", f.description);
}
...
fn test_basic_auth_over_http() {
    ...
    let f = report.findings.iter().find(|f| f.header == "www-authenticate").unwrap();
    assert_eq!(f.severity, Severity::High);
    assert!(f.description.contains("cleartext"), "desc={}", f.description);

    // Over HTTPS — severity Medium
    let report2 = analyze_headers(&headers, "https://example.com");
    let f2 = report2.findings.iter().find(|f| f.header == "www-authenticate").unwrap();
