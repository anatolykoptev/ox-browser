...
pub fn analyze_cookies(set_cookie_headers: &[String], page_url: &str) -> CookieReport {
    ...
    // Anti-CSRF cookies without SameSite
    const CSRF_PATTERNS: &[&str] = &["csrf", "xsrf", "_token", "antiforgery"];

    for c in &cookies {
        let lower = c.name.to_lowercase();
        let is_csrf = CSRF_PATTERNS.iter().any(|p| lower.contains(p));
        if is_csrf && c.same_site.is_none() {
            findings.push(CookieFinding {
                cookie: c.name.clone(),
                description: "CSRF token cookie without SameSite attribute".into(),
                severity: Severity::Medium,
            });
        }
    }
