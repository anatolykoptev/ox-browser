...
impl FlowState {
    ...
    /// Insert x-client-transaction-id header if available.
    async fn with_xtid(&self, mut headers: HeaderMap, url: &str) -> HeaderMap {
    ...
    pub async fn init(
        ...
        let headers = state.with_xtid(state.headers(), &url).await;
    ...
    pub async fn execute_task(
        ...
        let body =
            api_subtasks::task_body(&self.flow_token, subtask_data);
        let url = format!("{API_BASE}{ONBOARDING_TASK}");
        let headers = self.with_xtid(self.headers(), &url).await;
    ...
    pub async fn js_instrumentation(
        &mut self,
        client: &wreq::Client,
    ) -> Result<(), TwitterLoginError> {
        self.execute_task(client, api_subtasks::js_instrumentation())
            .await
    }
    ...
    pub async fn enter_username(
        ...
        self.execute_task(client, api_subtasks::enter_username(username))
            .await
    }
    ...
    pub async fn enter_password(
        ...
        self.execute_task(client, api_subtasks::enter_password(password))
            .await
    }
    ...
    pub async fn enter_text(
        ...
        self.execute_task(client, api_subtasks::enter_text(subtask_id, text))
            .await
    }

    pub async fn duplication_check(
        &mut self,
        client: &wreq::Client,
    ) -> Result<(), TwitterLoginError> {
        self.execute_task(client, api_subtasks::duplication_check())
            .await
    }
}
